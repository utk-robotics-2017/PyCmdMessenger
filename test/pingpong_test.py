...
class CharTest(Test):
    ...
    def print_results(self, sent_value, received_value, status):
        try:
            print(b" .... " + sent_value.encode("raw-unicode-escape") + b' --> ' +
                  received_value.encode("raw-unicode-escape") + b' : ' + status.encode("ascii"))
        except:
            print(" .... {:20} --> {:20} : {:20}".format(sent_value, received_value, status))
...
def main(argv=None):
    ...
    t = PingPong("kBInt32", board, long_test)
    ...
    t = PingPong("kInt32", board, long_test)
    string_trials.extend(t.run_tests())
